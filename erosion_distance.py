	...
	for i in range(n-1,-1,-1):
		for j in range(n-1,-1,-1):
			for k in range(n-1,-1,-1):
				if i!=n-1 and j!=n-1 and k!=n-1 and i+j<n:
					fshifts[i,j,k]=elementshift(newf,newg,i,j,k,fshifts[i+1,j+1,k+1]+1)
					gshifts[i,j,k]=elementshift(newg,newf,i,j,k,gshifts[i+1,j+1,k+1]+1)
		...
		for i in range(n):
			...
			for j in range(n):
				if currentx >= mx-1:
					...
					#print(i,j,k,currentz,currenty,mx-1,f[currentz,currenty,mx-1])
				elif tfx[currentx+1] == nt[j]:
					currentx +=1
					newf[j,i,k] = f[currentx,currenty,currentz]
					#print(i,j,k,currentz,currenty,currentx,f[currentz,currenty,currentx])
				else:
					newf[j,i,k] = f[currentx,currenty,currentz]
					#print(i,j,k,currentz,currenty,currentx,f[currentz,currenty,currentx])
...
def elementshift(f,g,i,j,k,old):
	[i1, j1, k1] = np.shape(f)
	if old == -1:
		r = min([i1-i, j, k])
	else:
